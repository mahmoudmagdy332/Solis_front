...
const Blogs = () => {
  ...
  const { translations, lang } = useLanguageSelector(
  ...
  return (
    <div className=" relative markets">
      ...
      <div className="w-screen mx-auto  py-4 flex  flex-col gap-6 items-center">
                  ...
                  <div className=" flex flex-col gap-2 justify-center  card-blog hover:shadow-xl bg-black text-white  h-84 border-1  rounded-md ">
                    ...
                    <div className="px-4 py-6 flex flex-col gap-4">
                      <div className="flex gap-2 items-center">
                        <div className="bg-white px-2 py-1 text-black text-nowrap truncate">
                          {servese.blog_category?.name}
                        </div>
                        <p className="text-sm text-nowrap ml-auto">
                          {format(servese.updated_at, "MMMM dd, yyyy", {
                            locale: lang == "ar" ? ar : enUS,
                          })}
                        </p>
                      </div>
                      <h3 className="text-xl font-bold //text-nowrap //line-clamp-2 truncate">{servese.title}:</h3>
                      <p className="text-gray-300 text-sm leading-6 font-thin line-clamp-3">
                        {servese.card_description}
                      </p>
                      <div className="flex justify-start">
                        <Link
                          to={`/blog/${servese.id}`}
                          className="flex items-center text-md gap-2 border border-white rounded-sm px-4 py-2 hover:bg-white hover:text-black transition-all ease-in-out"
                        >
        ...
        <Link
          to={`/blogs`}
          className="text-center px-8 py-3 rounded-full flex justify-center gap-3 items-center  border-2 border-transparent transition-all duration-500 hover:bg-black hover:text-white"
        >
